...
import { WebSurface, PolicyError, SurfaceTimeoutError } from '../src/surface/web.js'
...
//
// F52: a single, one-shot surface.resolve() right after open() is deliberately
// used below rather than a retrying/polling helper. A child <frame src="..."> is
// not guaranteed to have committed its own navigation (and so report its real
// name()/url()) by the time the parent frameset's own "domcontentloaded" fires —
// open() itself is now responsible for waiting that out (see F52 in
// src/surface/web.ts), so a test that papers over the race with its own retry
// loop would never notice if that responsibility regressed.
describe('the frameset at /', () => {
  it('resolves the member number box inside the "main" child frame, not the top document', async () => {
    ...
    const r = await surface.resolve({ role: 'textbox', name: 'Member No.', framePath: ['main'], fallbacks: [] })
    expect(r.kind).toBe('one')
  ...
  it('fills, clicks Inquire and reads the balance entirely inside "main" while the top-level URL stays at /', async () => {
    await surface.open(base + '/')
    const box = await surface.resolve({ role: 'textbox', name: 'Member No.', framePath: ['main'], fallbacks: [] })
    if (box.kind !== 'one') throw new Error('box not resolved')
    await surface.act('fill', box.node, '40021')
    const btn = await surface.resolve({ role: 'button', name: 'Inquire', framePath: ['main'], fallbacks: [] })
    if (btn.kind !== 'one') throw new Error('button not resolved')
    await surface.act('click', btn.node)
  ...
  // F52: this is the test that actually distinguishes a fixed open() from a
  // broken one. `?inject=slow-frame` delays the "main" child's own response by
  // 3s; a single, non-retrying resolve() immediately after open() returns can
  // only see the textbox if open() itself waited for that child to commit.
  // Confirmed by hand that reverting the child-frame wait in
  // src/surface/web.ts's open() turns this test red — resolve() comes back
  // 'none' because the "main" frame still hasn't committed its navigation —
  // before restoring the fix. See wave3-report.md.
  it('a one-shot resolve immediately after open() finds the textbox even when the "main" frame is slow to load', async () => {
    await surface.open(base + '/?inject=slow-frame')
    const r = await surface.resolve({ role: 'textbox', name: 'Member No.', framePath: ['main'], fallbacks: [] })
    expect(r.kind).toBe('one')
  }, 15_000)
})
